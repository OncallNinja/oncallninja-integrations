...
class KibanaClient(ActionRouter):
    def __init__(
        ...
        if api_key:
            ...
            self.headers["Authorization"] = f"Basic {base64.b64encode(f'{username}:{password}'.encode()).decode()}"
        else:
            self.auth = None
        ...
        if cloud_id:
            ...
            if elasticsearch_url:
                self.elasticsearch_base_url = elasticsearch_url.rstrip('/')
            if kibana_url:
                self.kibana_base_url = kibana_url.rstrip('/')
